...
| ⑤ | `game-design-to-report` | 整合前序产出 → HTML 或 PPT（8 套设计系统 · ppt-master SVG 工作流逐页精制） | 入口选择后一次性生成 |
...
│ 机制创新  │     │ draw.io   │    │ 数值设计  │     │ 排期汇总 │     │ ppt-master │
...
运行时选择输出格式：**HTML**（自包含单文件，可直接打印）或 **PPT**（ppt-master SVG 工作流）。

> PPT 工作流蒸馏自 [ppt-master](https://github.com/hugohe3/ppt-master)（作者：何雨果 Hugo He）—— AI 驱动的 SVG→PPTX 原生可编辑流水线。

**核心流程**：策略师八项确认 → 图片获取 → SVG 逐页手写 → 质量门 → 后处理 → 导出原生可编辑 PPTX

**8 套完整设计系统**（蒸馏自 ui-ux-pro-max），策略师阶段自动匹配：
...
**八大创新**（ppt-master 核心机制）：
- **串行流水线** — 每一步输出是下一步输入，严禁跨阶段打包
- **先设计后执行** — 策略师八项确认（⛔ 阻塞），用户确认后自动推进
- **design_spec.md + spec_lock.md 双产物** — 设计叙事 + 执行契约
- **逐页手写 SVG** — 主 agent 逐页生成，每页前重读锁文件抗上下文漂移
- **每页前重读 spec_lock.md** — 色彩/字体/图标全部来自锁文件，不从记忆取
- **质量门** — error 必须修复，warning 记录继续
- **原生 DrawingML PPTX** — 每个形状、文本框、图表在 PowerPoint 中可直接编辑

**汇报对象适配**：根据内部团队 / 投资人 / 发行商 / 合作方自动调整侧重点和语气。

**质量保障**：内置 B4 质量检查清单，覆盖内容完整性和视觉一致性。
...
- **平台**：Reasonix Code
- **依赖**：HTML 模式使用内置工具；PPT 模式无需额外依赖（SVG 手写 + 直接导出）
