...
Reasonix Code 游戏设计五件套——从系统策划 → 原型图 → 正式策划案(GDD) → 资源清单 → HTML 汇报文档，一站式输出。
...
| ⑤ | `game-design-to-html` | 整合前序产出 → 自包含精美 HTML 汇报文档（8 套设计系统可选） | 一次性生成 |
...
┌──────────┐     ┌──────────┐     ┌──────────┐     ┌──────────┐     ┌──────────┐
│ 系统拆解  │ →→→ │ 风格适配  │ →→→ │ 文档概述  │ →→→ │ 美术需求 │ →→→│ 设计系统  │
│ 方案设计  │     │ UI 线框图 │     │ 系统设计  │     │ 音频需求 │    │ 章节编排  │
│ 数值框架  │     │ 交互流程  │     │ 交互与UI  │     │ 特效需求 │    │ 排版渲染  │
│ 机制创新  │     │ draw.io   │    │ 数值设计  │     │ 排期汇总 │     │ 打印优化  │
│ 初稿Excel │     │ 原型图    │    │ 美术需求  │     │ 外包建议 │     │ 单文件输出 │
└──────────┘     └──────────┘     └──────────┘     └──────────┘     └──────────┘
```
...
整合正式 GDD + 原型图 + 资源清单 → **自包含单文件 HTML**，可直接打印或浏览器展示。
