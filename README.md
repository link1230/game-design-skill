...
| ⑤ | `game-design-to-report` | 整合前序产出 → HTML 或 PPT（8 套设计系统 · Prezentit API） | 入口选择后一次性生成 |
...
│ 机制创新  │     │ draw.io   │    │ 数值设计  │     │ 排期汇总 │     │ Prezentit │
│ 初稿Excel │     │ 原型图    │    │ 美术需求  │     │ 外包建议 │     │ python-pptx│
└──────────┘     └──────────┘     └──────────┘     └──────────┘     └──────────┘
```
...
运行时选择输出格式：**HTML**（自包含单文件，可直接打印）或 **PPT**（通过 Prezentit API 生成 .pptx 下载）。
